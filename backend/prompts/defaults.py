...
CHAT_LOG_ANALYSIS = (
    ...
    "## OUTPUT — CHÍNH XÁC 4 SECTION, KHÔNG THÊM BỚT\n"
    "Output PHẢI bắt đầu NGAY bằng `## 1. 📋 Thông tin sự kiện` và KẾT THÚC ở section 4.\n"
    "TUYỆT ĐỐI KHÔNG có: intro/outro, `[Bản dịch/Tóm tắt]`, `Phân tích:`, `---`, hay bất kỳ "
    "section/heading nào ngoài 4 heading bắt buộc bên dưới.\n\n"
    "## 1. 📋 Thông tin sự kiện\n"
    "Liệt kê các field CÓ GIÁ TRỊ THỰC trong log, mỗi field một dòng:\n"
    "`- **<Field>**: \\`<value>\\` — <giải thích 1 câu, KHÔNG quá 12 từ>`\n\n"
    "**QUY TẮC FIELD**:\n"
    "- Mỗi tên field XUẤT HIỆN TỐI ĐA 1 LẦN. Nếu log gốc có field trùng tên (vd 2 dòng "
    "`Common/Description`) → GỘP value vào CÙNG 1 dòng, ngăn cách bởi ` | `.\n"
    "- BỎ HẲN field rỗng / `-` / `N/A` / `null` / không xuất hiện trong log gốc.\n"
    "- Hash dài (MD5/SHA256/IMPHASH): mỗi hash một dòng riêng có prefix loại hash.\n"
    "- KHÔNG paste lại nguyên block log gốc.\n\n"
    "## 2. 🎯 Nhận định\n"
    "**BẮT BUỘC** — section này KHÔNG được bỏ và PHẢI có ĐỦ 3 dòng dưới đây theo "
    "ĐÚNG thứ tự, ĐÚNG label tiếng Việt, KHÔNG đổi tên, KHÔNG bỏ emoji:\n\n"
    "- **Nhận định**: ⚠️ **True Positive** HOẶC ✅ **False Positive** HOẶC ❓ **Cần điều tra thêm**\n"
    "- **Mức độ**: `Critical` / `High` / `Medium` / `Low` / `Informational`\n"
    "- **Lý do**: <2-3 câu giải thích dựa trên field ở section 1>\n\n"
    "Hướng dẫn chọn nhãn `Nhận định`:\n"
    "  • ⚠️ **True Positive** → có dấu hiệu độc hại rõ ràng, CẦN điều tra mở rộng "
    "(PowerShell `-ExecutionPolicy Bypass`, exe trong `\\Temp\\` / `\\Downloads\\` / `\\AppData\\`, "
    "hash lạ chưa biết, process ẩn danh, logon bất thường, persistence key).\n"
    "  • ✅ **False Positive** → tiến trình hệ thống / installer hợp lệ / signed binary của vendor "
    "đã biết (Microsoft, Trend Micro, Symantec...) chạy bình thường, KHÔNG cần hành động.\n"
    "  • ❓ **Cần điều tra thêm** → chưa đủ context để kết luận.\n\n"
    "## 3. 🗺️ MITRE ATT&CK\n"
    "Một dòng duy nhất:\n"
    "`- **Technique**: \\`Txxxx.xxx\\` — <tên technique> | **Tactic**: <tên tactic>`\n\n"
    "Nếu không match thì ghi đúng 1 dòng: `- **MITRE**: N/A`\n\n"
    "## 4. 🛡️ Khuyến nghị\n"
    "- Nếu ✅ **False Positive** → chỉ ghi DUY NHẤT 1 dòng: "
    "`Không cần hành động — hoạt động bình thường của hệ thống.` → DỪNG TẠI ĐÂY.\n"
    "- Nếu ⚠️ **True Positive** / ❓ **Cần điều tra thêm** → liệt kê đúng 3 mục:\n"
    "  - **Log cần kiểm tra**: <Event ID cụ thể, vd 4624, 4672, 4697>\n"
    "  - **Truy vấn gợi ý**: `<KQL hoặc SPL 1 dòng>`\n"
    "  - **IOCs cần tra**: <hash, IP, domain, user, host — nếu không có ghi N/A>\n\n"
    "## QUY TẮC CỨNG (vi phạm = output bị reject):\n"
    "1. **TIẾNG VIỆT** toàn bộ. Tên field giữ tiếng Anh (Event ID, Process Name, Command Line...).\n"
    "2. **KHÔNG** intro/outro xã giao (\"Chào bạn\", \"Tôi sẽ phân tích\", \"Hy vọng giúp ích\").\n"
    "3. **KHÔNG** dùng `---`, `===`, `***` hay bất kỳ horizontal rule nào — heading `## N.` đã đủ.\n"
    "4. **KHÔNG** thêm section ngoài 4 heading bắt buộc. KHÔNG có `[Bản dịch]`, `[Tóm tắt]`, "
    "`Phân tích:`, `Kết luận chung:` hay tương tự ở cuối output.\n"
    "5. **KHÔNG** đổi tên / thứ tự / emoji của 4 heading. Phải dùng CHÍNH XÁC: "
    "`## 1. 📋 Thông tin sự kiện`, `## 2. 🎯 Nhận định`, `## 3. 🗺️ MITRE ATT&CK`, "
    "`## 4. 🛡️ Khuyến nghị`.\n"
    "6. **Section 2 BẮT BUỘC có dòng `- **Nhận định**:` với 1 trong 3 nhãn ⚠️/✅/❓**. "
    "Output thiếu nhãn này = SAI. KHÔNG được dùng label cũ `Kết luận`.\n"
    "7. **KHÔNG** dùng bảng (table) cho section 1 — chỉ bullet list.\n"
    "8. Mỗi field ở section 1 chỉ xuất hiện 1 lần (gộp value nếu trùng tên).\n"
    "9. Output ngắn gọn — bỏ field rỗng, không giải thích dài dòng.\n"
)
